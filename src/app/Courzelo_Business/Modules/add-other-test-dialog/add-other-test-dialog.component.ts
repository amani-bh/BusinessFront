...
import { QuestionsOtherTest } from '../../Shared/entities/OtherTest';
...
@Component({
  selector: 'app-add-other-test-dialog',
  templateUrl: './add-other-test-dialog.component.html',
  styleUrls: ['./add-other-test-dialog.component.css']
})
export class AddOtherTestDialogComponent implements OnInit {
  ...
  constructor(private sanitization:DomSanitizer,private uploadService:FileUploadService,private _location: Location, private testsService: OtherTestService, private fb: FormBuilder, private businesstokenStorage: BusinessTokenStorageService) { }
    ...
    if (form.invalid) {
      ...
      for (let i = 0; i < form.get('responses').length; i++) {
        ...
        if (form.get('responses').at(i).get('state').value === true) {
          form.get('correctResponses').value.push(form.get('responses').at(i).value.label)
          
        }
        else if (form.get('responses').at(i).get('state').value === false) {
          form.get('falseResponses').value.push(form.get('responses').at(i).value.label)
        }
...
GetImage(i:any,j:any){  
  if(this.imageUrl[j]){this.Questions().controls[i].value.responses[j].label=this.imageUrl[j].url}{   
